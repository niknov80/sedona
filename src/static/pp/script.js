function pixelGlass() {
  ...
  'use strict';
  const OVERFLOW_TEST_PHRASE = "У меня есть несколько клиентов, которые записались ко мне на стрижку, не успев даже зайти в салон.";
  ...
  var overflowList = ['normal', 'overflow'];
  ...
  var currents = {
    ...
    overflow: getCurrent('overflow', overflowList[0])
  };
  ...
  // Overflow switcher params
  var paramsOverflow = {
    elemTag: 'button',
    elemText: 'overflow',
    listName: 'overflow',
    itemName: 'overflow',
    target: targets.overflow,
    type: 'button',
    list: overflowList,
    attrs: {
      tabindex: 4,
  ...
  function createButton(params) {
    ...
    input.onclick = function () {
      ...
      if (currentVal === 'overflow') {
        getTextNodesIn(document.body).forEach((item) => {
          item.textContent = OVERFLOW_TEST_PHRASE;
        })
      }
